...
func getLockFileInfo(lockPath string) *LockFileInfo {
	...
	if stat, err := os.Stat(lockPath); err == nil {
		...
		// Try to determine if file is locked (platform-specific)
		if file, err := os.OpenFile(lockPath, os.O_RDWR, 0); err == nil {
			defer file.Close()
			info.Locked = isFileLocked(file)
		}
	}
...
// isFileLocked checks if a file is locked in a cross-platform way
// On Unix systems, uses flock; on Windows, returns false (best effort)
func isFileLocked(file *os.File) bool {
	if runtime.GOOS == "windows" {
		// On Windows, file locking detection is more complex
		// For now, assume files are not locked (best effort)
		return false
	}

	// Unix systems: use flock
	return isFileLockedUnix(file)
}
